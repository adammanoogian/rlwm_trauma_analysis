...
Of the six choice-only models submitted to hierarchical Bayesian fitting, four converged (M2, M3, M5, M6a) and two were excluded. Q-learning (M1) failed catastrophically ($\hat{R} = 1.73$, ESS = 6), consistent with fundamental model misspecification for tasks with working-memory demands [@collins2014working]. M6b failed the ESS gate ($\hat{R} = 1.05$, ESS$_{\min}$ = 50, 0 divergences, BFMI = 0.74) despite 100\% PPC coverage (17/17 metrics); the dual-$\kappa$ stick-breaking decomposition creates posterior correlations that mix poorly under the single-$\alpha$ parameterization, and the capacity parameter was also poorly identified (ICC = 0.10).

PSIS-LOO stacking weights were unreliable: Pareto-$k$ diagnostics showed 58--60\% of observations exceeding $k > 0.7$ across all four converged models, causing all ELPD estimates to be NaN and stacking weights to collapse to uniform (0.25 each; @tbl-loo-stacking). This level of Pareto-$k$ failure indicates that importance sampling cannot bridge the gap between the full-posterior and leave-one-out predictive distributions, likely because individual-level parameters in the hierarchical model are too strongly informed by their own data [@vehtari2017practical]. K-fold cross-validation would be the principled remedy but requires $K \times 4$ complete refits and was not feasible within the current compute budget.

The secondary ranking via random-effects BMS [@stephan2009bayesian; @rigoux2014bayesian] produced interpretable results. The Bayesian Omnibus Risk was effectively zero (BOR $\approx$ 0), indicating strong evidence that models differ at the population level. Protected exceedance probabilities favoured M6a (stimulus-specific perseveration; PXP = 0.714) over M5 (RL forgetting; PXP = 0.286), with M2 and M3 receiving PXP $\approx$ 0 (@tbl-rfx-bms). This ranking diverges from the MLE AIC/BIC ranking where M5 was the winner ($\Delta$AIC = 435.6 over M3).

**Shrinkage diagnostics explain the ranking discrepancy.** Hierarchical estimation applies adaptive regularization that differs fundamentally from AIC's flat $2k$ penalty. The shrinkage diagnostic [@gelman2013bayesian] quantifies how much the group-level prior constrains individual-level estimates, ranging from 0 (no pooling; individual estimates are data-only) to 1 (complete pooling; all individuals collapse to the group mean). In M5, the RL-forgetting parameter $\phi_{\text{rl}}$ had shrinkage = 0.084, indicating that the between-subject variance $\sigma_{\phi_\text{rl}}$ was so large relative to the individual-level posteriors that the hierarchy provided almost no regularization. Effectively, $\phi_{\text{rl}}$ added individual-level complexity without contributing to population-level prediction — reducing M5 to approximately M3 plus noise on the $\phi_{\text{rl}}$ dimension. By contrast, M6a's stimulus-specific perseveration kernel $\kappa_s$ had shrinkage = 0.909, meaning the population distribution was tight and the hierarchy efficiently leveraged it for group-level inference. AIC, which treats all parameters equally regardless of identifiability, rewarded M5's $\phi_{\text{rl}}$ for improving per-participant fit; RFX-BMS, which operates on marginalized per-participant log-evidence, penalized it for failing to generalize across the population (@tbl-shrinkage-diagnostics).

: Shrinkage diagnostics for converged models. Values near 0 indicate poor hierarchical identifiability; values near 1 indicate strong group-level constraint. {#tbl-shrinkage-diagnostics}

| Parameter | M2 | M3 | M5 | M6a |
|-----------|-----|-----|-----|------|
| $\alpha$ | 0.55 | 0.58 | 0.64 | 0.57 |
| $\phi$ (WM decay) | 0.48 | 0.52 | 0.52 | 0.51 |
| $\rho$ (WM weight) | 0.46 | 0.51 | 0.52 | 0.51 |
| $K$ (capacity) | 0.00 | 0.21 | 0.00 | 0.20 |
| $\kappa$ | — | 0.91 | 0.91 | — |
| $\kappa_s$ | — | — | — | 0.91 |
| $\phi_{\text{rl}}$ | — | — | **0.08** | — |
| $\varepsilon$ | 0.65 | 0.65 | 0.66 | 0.66 |

The LOO + stacking ranking is in @tbl-loo-stacking; the random-effects BMS PXP is in @tbl-rfx-bms; the winner forest plots are in @fig-forest-21; and the complete winner Level-2 beta coefficients with 95% HDI and FDR-BH flag are in @tbl-winner-betas (see @sec-bayesian-regression).

```{python}
#| label: tbl-loo-stacking
#| tbl-cap: "PSIS-LOO stacking weights for converged choice-only models (M2, M3, M5, M6a). M1 (Q-learning) and M6b were excluded for non-convergence; M4 (joint choice+RT) is on a separate track. All ELPD estimates are NaN due to Pareto-$k$ diagnostic failure (58--60\\% of observations with $k > 0.7$); weights collapsed to uniform."

from pathlib import Path
import pandas as pd
_loo_path = Path("../reports/tables/model_comparison/table1_loo_stacking.csv")
if _loo_path.exists():
...
```{python}
#| label: tbl-rfx-bms
#| tbl-cap: "Random-effects Bayesian model selection: protected exceedance probabilities (PXP) for converged choice-only models (M2, M3, M5, M6a). BOR $\\approx$ 0 indicates strong evidence that models differ. M6a (stimulus-specific perseveration) received the highest PXP."
...
```{python}
#| label: fig-forest-21
#| fig-cap: "Forest plot of the stacking-winner model from the hierarchical Bayesian pipeline. Beta coefficients (probit scale) for each trauma predictor across all parameter sites. Thick bars = 50% HDI; thin bars = 95% HDI. Effects whose 95% HDI excludes zero are starred."
#| fig-width: 7.0
#| fig-height: 5.0
...
#| tbl-cap: "Hierarchical Bayesian Level-2 beta coefficients for the stacking-winner model. Each row is one parameter-predictor pair. 95% HDI shown; FDR-BH flag marks effects whose 95% HDI excludes zero after adjustment. Pending hierarchical Bayesian pipeline completion."
...
**Identifiability limitations.** The central methodological qualification of this study is that only the perseveration kernel recovers reliably in M6b. Individual differences in $K$ (WM capacity), $\phi$ (WM decay), $\rho$ (WM reliability), $\alpha$, and $\varepsilon$ cannot be interpreted as stable traits at the per-participant level, because recovery from synthetic data falls substantially short of the $r \geq 0.80$ criterion (Section \ref{sec-parameter-recovery}). Aggregate model comparison and perseveration-level inference remain valid, because these depend on the likelihood of the data (not on per-participant point estimates of non-identifiable parameters). The hierarchical Bayesian pipeline confirmed this pattern: shrinkage diagnostics showed that $K$ (shrinkage $\approx$ 0 in M2 and M5) and $\phi_{\text{rl}}$ (shrinkage = 0.084 in M5) were poorly identified even with partial pooling, while the perseveration kernel $\kappa$ was strongly identified (shrinkage $\geq$ 0.91 across all models that include it; @tbl-shrinkage-diagnostics). This convergent evidence from MLE recovery and Bayesian shrinkage strengthens the conclusion that perseveration-level inference is trustworthy while base RLWM parameter inference should be treated with caution.

**Other limitations.** (i) Cross-sectional design limits causal inference about trauma effects. (ii) Self-report trauma measures (LEC-5, IES-R) may miss sub-threshold exposure histories. (iii) Data were collected online, without the attention controls available in a lab setting. (iv) Set sizes were limited to 2, 3, 5, and 6; set size 4 was excluded by design following @senta2025. (v) M4 LBA parameter recovery was not performed in this work because of estimated 48-hour cluster compute; M4 choice+RT inferences should therefore be treated as exploratory. M4 uses a fundamentally different likelihood domain from the choice-only models, making direct comparison via AIC, LOO, or stacking weights inappropriate; we report M4's LOO diagnostics on a separate track. (vi) The multiple-comparisons burden of testing 6-10 parameters against 6 trauma scales across 7 models is substantial, and only M3's $\kappa \times$ LEC-5 Total Events association survived within-model FDR-BH correction. Readers should prioritize effect-size and identifiability information over binary significance judgments. (vii) Working-memory capacity $K$ has a restricted effective range ($[2, 6]$) dictated by the task's set-size structure [@collins2012how]; this limits precision of between-participant comparisons on $K$, although the fundamental constraint is inherent to the task. (viii) The IES-R subscale design matrix required Gram--Schmidt orthogonalization against the total score to avoid perfect collinearity; the hyperarousal residual was excluded due to exact linear dependence (the three subscales sum to the total), so subscale predictors capture unique variance conditional on the total score being partialled out. (ix) PSIS-LOO stacking weights were unreliable for all hierarchical models due to Pareto-$k$ diagnostic failure (58--60\% of observations with $k > 0.7$), necessitating reliance on RFX-BMS for Bayesian model ranking and AIC/BIC for the primary model selection criterion. The Pareto-$k$ failure likely reflects the strong influence of individual data on individual-level hierarchical parameters, a known challenge in cognitive models with many per-participant parameters [@vehtari2017practical]. K-fold cross-validation would resolve this but was computationally prohibitive. (x) The MLE and Bayesian rankings disagreed (M5 by AIC/BIC vs. M6a by RFX-BMS PXP), and M6b failed hierarchical convergence entirely; the model selection conclusion should therefore be interpreted with caution pending replication with longer MCMC chains or alternative estimation strategies.
...
We fitted seven computational models of reinforcement learning and working memory to behavioral data from `{python} n_participants` participants stratified by trauma history and symptom severity. MLE model selection favoured `{python} winner_display` by AIC and BIC. Hierarchical Bayesian model comparison via RFX-BMS favoured M6a (stimulus-specific perseveration; PXP = 0.714) over M5 (PXP = 0.286); PSIS-LOO stacking weights were unreliable due to pervasive Pareto-$k$ diagnostic failure. The most robust trauma-parameter association was a positive link between the perseveration kernel ($\kappa$) and lifetime trauma exposure (LEC-5 Total Events), which survived FDR-BH correction in M3 and approached threshold in M6b. By contrast, learning rates did not differ by trauma status at any correction level, tentatively favouring a motor-perseveration account of trauma-related behavioural inflexibility over a pure outcome-insensitivity account. These findings should be considered hypothesis-generating given the identifiability limitations of the base RLWM parameters, the moderate sample size, and the discrepancy between MLE and Bayesian model rankings. Replication in an independent, pre-registered sample --- ideally with a within-subject manipulation of perseveration cost --- is needed before clinical translation.
...
Bayesian model comparison via PSIS-LOO was attempted but Pareto-$k$ diagnostics indicated unreliable importance sampling (58--60\% of observations with $k > 0.7$; see @sec-bayesian-selection). Stacking weights collapsed to uniform and ELPD estimates were NaN (@tbl-stacking-weights). Random-effects BMS provided an alternative ranking, favouring M6a (PXP = 0.714) over M5 (PXP = 0.286); see @tbl-rfx-bms.
...
stacking_path = Path("../models/bayesian/level2/stacking_weights.csv")
