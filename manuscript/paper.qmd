...
Scale distributions and pairwise correlations are summarized in @fig-scale-distributions and documented in `docs/SCALES_AND_FITTING_AUDIT.md`.

::: {#fig-scale-distributions}
![](figures/scale_distributions.png){width="100%"}

Distributions and pairwise correlations of the trauma predictors. **Left**: histograms of LEC-5 total events and IES-R total and subscale scores. **Right**: Pearson correlation matrix. LEC total correlates only weakly with IES total ($r$ = 0.13), justifying inclusion of both predictors. IES subscales correlate strongly with IES total ($r$ = 0.91--0.93), requiring the Gram--Schmidt residualization described in Model Fitting. Figure produced by `scripts/analysis/trauma_scale_distributions.py` and copied into `manuscript/figures/` on pipeline rerun.
:::
...
try:
    ...
    print(f"Data not yet available. Run scripts/06_fit_analyses/compare_models.py first. ({e})")
```
...
if stacking_path.exists():
    ...
    print("Stacking weights not available. Run: python scripts/06_fit_analyses/compare_models.py --bayesian-comparison")
```
