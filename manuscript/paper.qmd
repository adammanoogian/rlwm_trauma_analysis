...
The secondary ranking via random-effects BMS [@stephan2009bayesian; @rigoux2014bayesian] produced interpretable results. The Bayesian Omnibus Risk was effectively zero (BOR $\approx$ 0), indicating strong evidence that models differ at the population level. Protected exceedance probabilities favoured M6a (stimulus-specific perseveration; PXP = 0.714) over M5 (RL forgetting; PXP = 0.286), with M2 and M3 receiving PXP $\approx$ 0 (@tbl-rfx-bms). This ranking diverges from the MLE AIC/BIC ranking where M5 was the winner ($\Delta$AIC = 435.6 over M3); the discrepancy may reflect the influence of hierarchical shrinkage on the relative penalization of model complexity.
