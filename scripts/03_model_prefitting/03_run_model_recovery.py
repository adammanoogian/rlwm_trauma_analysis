...
def run_recovery_for_model(
    ...
    print(f"{'=' * 60}")
    print(
        f"{'Parameter':<15} {'Pearson r':>10} {'RMSE':>10} {'Bias':>10} {'Status':>10}"
    )
    print(f"{'-' * 60}")
    ...
    for _, row in metrics_df.iterrows():
        ...
        if row["pass_fail"] != "PASS":
            all_pass = False
        print(
            f"{row['parameter']:<15} {row['pearson_r']:>10.3f} {row['rmse']:>10.3f} {row['bias']:>+10.3f} {status:>10}"
        )
...
def main():
    ...
    parser.add_argument(
        "--n-datasets", type=int, default=10, help="Number of independent datasets"
    )
    parser.add_argument(
        "--n-starts",
        type=int,
    ...
    if args.mode == "cross-model":
        ...
        if args.model == "all":
            ...
            print(
                "Error: M4 is excluded from cross-model recovery (joint likelihood "
