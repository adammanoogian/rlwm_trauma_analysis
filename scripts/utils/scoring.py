...
def score_less(survey1_df):
    ...
    any_exposure_cols = [
        col for col in result_df.columns if col.endswith("_any_exposure")
    ]
...
def score_ies_r(survey2_df):
    ...
    result_df["ies_total"] = (
        result_df["ies_intrusion"]
        + result_df["ies_avoidance"]
        + result_df["ies_hyperarousal"]
    )
...
def calculate_overall_task_metrics(task_df):
    ...
    metrics = {
        ...
        "accuracy_overall": task_df["correct"].mean()
        if "correct" in task_df.columns
        else np.nan,
        "mean_rt_overall": completed_trials["rt"].mean()
        if "rt" in completed_trials.columns
        else np.nan,
        "median_rt_overall": completed_trials["rt"].median()
        if "rt" in completed_trials.columns
        else np.nan,
    }
...
def calculate_load_condition_metrics(task_df):
    ...
    for load in ["low", "high"]:
        ...
        metrics[f"accuracy_{load}_load"] = (
            load_trials["correct"].mean() if len(load_trials) > 0 else np.nan
        )
        metrics[f"mean_rt_{load}_load"] = (
            completed_load["rt"].mean() if len(completed_load) > 0 else np.nan
        )
...
def calculate_set_size_metrics(task_df):
    ...
    for set_size in [2, 3, 5, 6]:
        ...
        metrics[f"accuracy_setsize_{set_size}"] = (
            ss_trials["correct"].mean() if len(ss_trials) > 0 else np.nan
        )
        metrics[f"mean_rt_setsize_{set_size}"] = (
            completed_ss["rt"].mean() if len(completed_ss) > 0 else np.nan
        )
...
def calculate_block_tercile_metrics(task_df):
    ...
    for period, block_range in [
        ...
        metrics[f"accuracy_{period}_blocks"] = (
            period_trials["correct"].mean() if len(period_trials) > 0 else np.nan
        )
        metrics[f"mean_rt_{period}_blocks"] = (
            completed_period["rt"].mean() if len(completed_period) > 0 else np.nan
        )
...
def calculate_learning_curve(task_df):
    ...
    """
    if "block" not in task_df.columns or len(task_df) == 0:
        return {"learning_slope": np.nan, "learning_improvement_early_to_late": np.nan}
    ...
    early_acc = (
        block_accuracy.iloc[:3].mean()
        if len(block_accuracy) >= 3
        else block_accuracy.iloc[0]
    )
    late_acc = (
        block_accuracy.iloc[-3:].mean()
        if len(block_accuracy) >= 3
        else block_accuracy.iloc[-1]
    )
    improvement = late_acc - early_acc

    return {"learning_slope": slope, "learning_improvement_early_to_late": improvement}
...
def calculate_reversal_metrics(task_df):
    ...
    reversal_positions = sorted_df.index[
        sorted_df.index.isin(reversal_indices)
    ].tolist()
    ...
    mean_post_immediate = (
        np.mean(post_reversal_acc_immediate)
        if len(post_reversal_acc_immediate) > 0
        else np.nan
    )
    mean_post_adapted = (
        np.mean(post_reversal_acc_adapted)
        if len(post_reversal_acc_adapted) > 0
        else np.nan
    )

    performance_drop = (
        mean_pre - mean_post_immediate
        if not np.isnan(mean_pre) and not np.isnan(mean_post_immediate)
        else np.nan
    )
    adaptation_rate = (
        mean_post_adapted - mean_post_immediate
        if not np.isnan(mean_post_adapted) and not np.isnan(mean_post_immediate)
        else np.nan
    )
