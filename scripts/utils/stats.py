...
try:
    ...
    warnings.warn("pingouin not installed. Run: pip install pingouin", stacklevel=2)
    pg = None
...
try:
    ...
    warnings.warn(
        "statsmodels not installed. Run: pip install statsmodels", stacklevel=2
    )
    sm = None
...
def check_homogeneity_of_variance(data, dv_col, group_col):
    ...
    return {"test": "Levene", "F": f_stat, "p": p_val, "homogeneous": p_val > 0.05}
...
def run_mixed_anova(data, dv, within_factor, between_factor, subject_id):
    ...
    if pg is None:
        raise ImportError(
            "pingouin is required for mixed ANOVA. Install with: pip install pingouin"
        )
...
def run_rm_anova(data, dv, within_factor, subject_id):
    ...
    if pg is None:
        raise ImportError(
            "pingouin is required for RM-ANOVA. Install with: pip install pingouin"
        )

    aov = pg.rm_anova(
        data=data, dv=dv, within=within_factor, subject=subject_id, correction=True
    )
...
def run_between_anova(data, dv, between_factor):
    ...
    if pg is None:
        raise ImportError(
            "pingouin is required for ANOVA. Install with: pip install pingouin"
        )

    aov = pg.anova(data=data, dv=dv, between=between_factor)
...
def run_welch_anova(data, dv, between_factor):
    ...
    if pg is None:
        raise ImportError(
            "pingouin is required for Welch ANOVA. Install with: pip install pingouin"
        )

    aov = pg.welch_anova(data=data, dv=dv, between=between_factor)
...
def post_hoc_tests(data, dv, between_factor, padjust="bonf"):
    ...
    if pg is None:
        raise ImportError(
            "pingouin is required for post-hoc tests. Install with: pip install pingouin"
        )

    return pg.pairwise_tests(
        data=data, dv=dv, between=between_factor, padjust=padjust, parametric=True
    )
