...
def extract_ies_scores(scored_responses_json):
    ...
    """
    if (
        pd.isna(scored_responses_json)
        or scored_responses_json == "{}"
        or scored_responses_json == ""
    ):
    ...
    try:
        ...
        item_scores = {
            f"s2_item{i:02d}": scores.get(f"s2_item{i:02d}", np.nan)
            for i in range(1, 23)
        }
        return item_scores
...
def extract_survey1_data(df):
    ...
    survey1 = survey1.drop(
        columns=[col for col in survey1_cols if col in survey1.columns]
    )
...
def extract_survey2_data(df):
    ...
    if "response" in survey2_data.columns and survey2_data["response"].notna().any():
        ...
        survey2 = (
            survey2_data.groupby("sona_id")["scored_responses"].first().reset_index()
        )
...
def extract_task_trials(df):
    ...
    if "key_press" in task_trials.columns:
        task_trials["key_press"] = pd.to_numeric(
            task_trials["key_press"], errors="coerce"
        )
...
def validate_data(df):
    ...
    if "sona_id" in df.columns and len(df) > 0 and df["sona_id"].duplicated().any():
