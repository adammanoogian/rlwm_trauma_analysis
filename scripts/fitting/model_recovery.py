...
def generate_synthetic_participant(
    params: dict[str, float], model: str, seed: int
) -> pd.DataFrame:
    """
    ...
    for block_idx in range(NUM_BLOCKS):
        ...
        if model in ["wmrl", "wmrl_m3", "wmrl_m5", "wmrl_m6a", "wmrl_m6b", "wmrl_m4"]:
            WM = np.ones((MAX_STIMULI, NUM_ACTIONS)) * (
                1.0 / NUM_ACTIONS
            )  # Baseline = 0.333
        ...
        reversal_threshold = rng.integers(
            MIN_CORRECT_FOR_REVERSAL, MAX_CORRECT_FOR_REVERSAL + 1
        )
        reward_mapping = rng.integers(
            0, NUM_ACTIONS, size=set_size
        )  # Random action per stimulus
        ...
        for trial_in_block in range(1, n_trials_block + 1):
            ...
            if model in [
                ...
                if model in ("wmrl_m3", "wmrl_m5"):
                    ...
                    eff_kappa_s = (
                        kappa_s if last_actions.get(stimulus) is not None else 0.0
                    )
            ...
            if model == "wmrl_m4":
                ...
                action = int(
                    jax.random.choice(subkey, NUM_ACTIONS, p=jnp.array(action_probs))
                )
                rt_sim = None
...
def compare_behavior(
    ...
    for source, data in [("real", real_data), ("synthetic", synthetic_data)]:
        ...
        if len(blocks) >= 14:
            ...
            row["accuracy_early"] = data[data["block"].isin(early_blocks)][
                "reward"
            ].mean()
            row["accuracy_late"] = data[data["block"].isin(late_blocks)][
                "reward"
            ].mean()
    ...
    comparison_df = pd.concat(
        [comparison_df, pd.DataFrame([diff_row])], ignore_index=True
    )
...
def run_posterior_predictive_check(
    ...
    synthetic_dfs = []
    iterator = (
        tqdm(fitted_params_list, desc="Generating synthetic data")
        if verbose
        else fitted_params_list
    )
    ...
    for params in iterator:
        ...
        seed = (
            int(sona_id)
            if isinstance(sona_id, (int, float))
            else hash(str(sona_id)) % 2**31
        )

        syn_df = generate_synthetic_participant(params=params, model=model, seed=seed)
    ...
    plot_behavioral_comparison(
        real_data, synthetic_data, figures_dir, model_name=model.upper()
    )
...
def run_parameter_recovery(
    ...
    for dataset_idx in dataset_iter:
        ...
        for subject_idx in subject_iter:
            ...
            df_synthetic = generate_synthetic_participant(
                true_params, model, subject_seed
            )
...
def plot_recovery_scatter(
    results_df: pd.DataFrame, metrics_df: pd.DataFrame, model: str, output_dir: Path
) -> None:
    """
    ...
    for idx, param in enumerate(param_names):
        ...
        ax_ind.set_title(
            f"{param.capitalize()} Recovery", fontsize=14, fontweight="bold"
        )
        fig_ind.tight_layout()
        fig_ind.savefig(
            output_dir / f"{param}_recovery.png", dpi=300, bbox_inches="tight"
        )
        plt.close(fig_ind)
    ...
    fig_combined.suptitle(
        f"{model.upper()} Parameter Recovery", fontsize=16, fontweight="bold"
    )
    fig_combined.tight_layout()
    fig_combined.savefig(
        output_dir / "all_parameters_recovery.png", dpi=300, bbox_inches="tight"
    )
    plt.close(fig_combined)


def plot_distribution_comparison(
    results_df: pd.DataFrame, real_params_path: str | None, model: str, output_dir: Path
) -> None:
    """
    ...
    for idx, param in enumerate(param_names):
        ...
        distributions = {"Recovered": recovered_vals, "Real Fitted": real_vals}
        ...
        ax.set_title(
            f"{param.capitalize()} Distribution", fontsize=12, fontweight="bold"
        )
    ...
    fig.suptitle(
        f"{model.upper()} Parameter Distributions", fontsize=16, fontweight="bold"
    )
    fig.tight_layout()
    fig.savefig(
        output_dir / "parameter_distributions.png", dpi=300, bbox_inches="tight"
    )
    plt.close(fig)
...
Examples:
  ...
  python scripts/fitting/model_recovery.py --model wmrl --n-subjects 10 --n-datasets 1 --seed 42
    ...
    parser.add_argument(
        "--output-dir",
        type=str,
        default=str(MODELS_RECOVERY_DIR),
        help="Output directory for results (default: models/recovery)",
    )
    parser.add_argument(
        "--figures-dir",
        type=str,
        default=str(REPORTS_FIGURES_DIR / "recovery"),
    ...
    if args.mode == "recovery":
        ...
        plot_distribution_comparison(
            results_df, args.real_params, args.model, figures_dir
        )
        ...
        if all_passed:
            ...
            failed_params = metrics_df[metrics_df["pass_fail"] == "FAIL"][
                "parameter"
            ].tolist()
            if not args.quiet:
...
def run_cross_model_recovery(
    ...
    confusion = pd.DataFrame(0, index=generating_models, columns=comparison_models)
...
def run_tests():
    ...
    results = run_parameter_recovery(
        "qlearning", n_subjects=2, n_datasets=1, seed=42, verbose=False
    )
