...
def compute_aggregate_ic(fits_df: pd.DataFrame, metric: str = "aic") -> float:
    ...
    """
    converged = fits_df[fits_df["converged"]]
    if len(converged) == 0:
        converged = fits_df
...
def count_winning_model(
    ql_fits: pd.DataFrame, wmrl_fits: pd.DataFrame, metric: str = "aic"
) -> dict[str, int]:
    """
    ...
    merged = pd.merge(
        ql_fits[["participant_id", metric, "converged"]].rename(
            columns={metric: f"{metric}_ql", "converged": "converged_ql"}
        ),
        wmrl_fits[["participant_id", metric, "converged"]].rename(
            columns={metric: f"{metric}_wmrl", "converged": "converged_wmrl"}
        ),
        on="participant_id",
        how="inner",
    )
    ...
    both_converged = merged[(merged["converged_ql"]) & (merged["converged_wmrl"])]

    n_ql_wins = (
        both_converged[f"{metric}_ql"] < both_converged[f"{metric}_wmrl"]
    ).sum()
    n_wmrl_wins = (
        both_converged[f"{metric}_wmrl"] < both_converged[f"{metric}_ql"]
    ).sum()
...
def compute_delta_ic(
    ...
    merged = pd.merge(
        ...
        wmrl_fits[["participant_id", metric]].rename(
            columns={metric: f"{metric}_wmrl"}
        ),
        on="participant_id",
        how="inner",
    )
...
def main():
    ...
    parser.add_argument(
        "--m2", type=str, default=None, help="Path to M2 (WM-RL) individual fits CSV"
    )
    parser.add_argument(
        "--m3",
        type=str,
    ...
    for model_name, fits_df in fits_dict.items():
        ...
        print(
            f"  {model_name}: {n_converged}/{len(fits_df)} ({100 * n_converged / len(fits_df):.1f}%)"
        )
    ...
    for _, row in aic_comparison.iloc[1:].iterrows():
        delta = row["delta_aic"]
        print(
            f"  {row['model']} vs {best_aic_model}: delta AIC = {delta:.2f} ({interpret_delta_aic(delta)})"
        )
    ...
    for model_name in sorted(weights.keys()):
        print(
            f"  {model_name}: {weights[model_name]:.4f} ({100 * weights[model_name]:.2f}%)"
        )
    ...
    if "M3" in fits_dict:
        ...
        m3_converged = m3_fits[m3_fits["converged"]]
        if "kappa" in m3_converged.columns:
            ...
            print(
                f"  kappa:       Mean = {kappa_mean:.3f}±{kappa_se:.3f}, Median = {kappa_median:.3f}"
            )
            print(
                f"               Range = [{m3_converged['kappa'].min():.3f}, {m3_converged['kappa'].max():.3f}]"
            )
    ...
    if args.output:
        ...
        comparison_results = pd.merge(
            aic_comparison.rename(
                columns=lambda x: x.replace("aic", "metric") if "aic" in x else x
            ),
