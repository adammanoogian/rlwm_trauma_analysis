...
def test_jax_likelihood_direct():
    ...
    rew_test = [jnp.array([1.0, 0.0, 1.0, 1.0, 0.0], dtype=jnp.float32)]

    # Note: beta is fixed at 50 inside the function, epsilon comes after alpha_neg
    ll_test = q_learning_multiblock_likelihood(
        stim_test, act_test, rew_test, 0.3, 0.1, 0.05
    )
...
if __name__ == "__main__":
    ...
    print(
        f"True parameters: alpha_pos={true_alpha_pos}, alpha_neg={true_alpha_neg}, epsilon={true_epsilon}"
    )
