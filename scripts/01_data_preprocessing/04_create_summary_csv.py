...
def main():
    ...
    insufficient_blocks = block_counts[
        block_counts < DataParams.MIN_BLOCKS
    ].index.tolist()
    insufficient_trials = trial_counts[
        trial_counts < DataParams.MIN_TRIALS
    ].index.tolist()
    ...
    print(
        f"Auto-excluded by trial count (<{DataParams.MIN_TRIALS}): {len(EXCLUDED_PARTICIPANTS)}"
    )
    ...
    task_trials_filtered = task_trials[
        ~task_trials["sona_id"].isin(EXCLUDED_PARTICIPANTS)
    ]
