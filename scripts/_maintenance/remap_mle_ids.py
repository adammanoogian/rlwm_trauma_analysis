...
def evaluate_qlearning_nll(block_data_pid, alpha_pos, alpha_neg, epsilon):
    ...
    stimuli_blocks = [
        block_data_pid[b]["stimuli"] for b in sorted(block_data_pid.keys())
    ]
    actions_blocks = [
        block_data_pid[b]["actions"] for b in sorted(block_data_pid.keys())
    ]
    rewards_blocks = [
        block_data_pid[b]["rewards"] for b in sorted(block_data_pid.keys())
    ]
...
def evaluate_wmrl_nll(
    block_data_pid, alpha_pos, alpha_neg, phi, rho, capacity, epsilon, set_sizes_blocks
):
...
def prepare_set_sizes(trial_df, pid, block_data_pid):
    ...
    for block_num in sorted(block_data_pid.keys()):
        ...
        if "set_size" in block_trials.columns and len(block_trials) > 0:
            set_sizes_blocks.append(
                jnp.array(block_trials["set_size"].values, dtype=jnp.float32)
            )
        else:
...
def remap_model(mle_csv_path, trial_df, block_data, model_type):
    ...
    print(
        f"  Direct matches: {len(direct_rows)} -> {list(direct_rows['participant_id'])}"
    )
    ...
    for _idx, row in tqdm(
        rows_to_remap.iterrows(), total=len(rows_to_remap), desc="  Matching"
    ):
...
def main():
    ...
    print(
        f"Trial data: {trial_df['sona_id'].nunique()} participants (after exclusions)"
    )
    ...
    if ql_path.exists():
        ql_mapping, ql_unmatched = remap_model(
            ql_path, trial_df, block_data, "qlearning"
        )
    ...
    if wmrl_path.exists():
        wmrl_mapping, wmrl_unmatched = remap_model(
            wmrl_path, trial_df, block_data, "wmrl"
        )
    ...
    print(
        f"Q-learning: {len(ql_mapping)} remapped + {n_direct} direct = {len(ql_mapping) + n_direct}/49"
    )
    print(
        f"WM-RL:      {len(wmrl_mapping)} remapped + {n_direct} direct = {len(wmrl_mapping) + n_direct}/49"
    )
