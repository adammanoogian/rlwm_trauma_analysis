    ...
    for model in CHOICE_ONLY_MODELS:
        ...
        if not path.exists():
            raise FileNotFoundError(
                ...
                f"Run `python scripts/04_model_fitting/a_mle/fit_mle.py --model {model}` first."
            )
...
def plot_winner_heterogeneity(combined: pd.DataFrame, output_path: Path) -> None:
    ...
    palette = dict(
        zip(
            observed_groups,
            sns.color_palette("husl", len(observed_groups)),
            strict=False,
        )
    )

    for ax, param in zip(axes_flat, parameters_to_plot, strict=False):
        column = f"m6b_{param}"
        sns.boxplot(
            data=combined,
            x="winning_model",
