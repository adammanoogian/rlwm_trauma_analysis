...
def compare_models_mle(
    fits_dict: dict[str, pd.DataFrame], metric: str = "aic"
) -> pd.DataFrame:
    """Compare N models using aggregate information criteria."""
    agg_ics = {
        model: compute_aggregate_ic(fits, metric) for model, fits in fits_dict.items()
    }
...
def count_participant_wins(
    ...
    for model_name, fits_df in fits_dict.items():
        ...
        model_df = model_df.rename(
            ...
            if "converged" in fits_df.columns
            else {metric: f"{metric}_{model_name}"}
        )
        ...
        summary.append(
            {
                ...
                "win_pct": 100 * win_counts.get(model_name, 0) / len(merged)
                if len(merged) > 0
                else 0,
                "total": len(merged),
            }
...
def plot_model_comparison(
    aic_comparison: pd.DataFrame, bic_comparison: pd.DataFrame, output_dir: Path
) -> None:
    ...
    # Add value labels
    for bar, val in zip(bars, aics, strict=False):
        ax.text(
    ...
    for bar, val in zip(bars, bics, strict=False):
        ax.text(
            bar.get_x() + bar.get_width() / 2,
            bar.get_height(),
            f"{val:.0f}",
...
def plot_model_weights(weights: dict[str, float], output_dir: Path) -> None:
    ...
    ax.set_title(
        "Model Weights (probability of being best model)",
        fontsize=14,
        fontweight="bold",
    )
    ax.set_ylim([0, 1.1])
    ...
    for bar, val in zip(bars, weight_vals, strict=False):
        ax.text(
            bar.get_x() + bar.get_width() / 2,
            bar.get_height() + 0.02,
            f"{val:.3f}",
...
def plot_participant_wins(wins_df: pd.DataFrame, metric: str, output_dir: Path) -> None:
    ...
    ax.set_title(
        f"Winning Model by {metric.upper()} (per participant)",
        fontsize=14,
        fontweight="bold",
    )
    ax.grid(True, alpha=0.3, axis="y")

    for bar, (w, pct) in zip(
        bars, zip(wins, wins_df["win_pct"], strict=False), strict=False
    ):
        ax.text(
            bar.get_x() + bar.get_width() / 2,
            bar.get_height(),
...
def stratified_comparison(
    ...
    groups_df["group"] = (
        ...
        .map(label_map)
        .fillna(groups_df["hypothesis_group"])
    )
    ...
    if len(unique_models) >= 2 and len(unique_groups) == 2:
        ...
        if ct_2x2.shape == (2, 2):
            ...
            if fisher_p < 0.05:
                ...
                print(
                    "    -> Not significant: no evidence model preference differs by group"
                )
        else:
            fisher_p = np.nan
            odds_ratio = np.nan
    ...
    for group_name in unique_groups:
        ...
        for model_name in fits_dict:
            ...
            print(
                f"    {model_name}: {metric.upper()}={group_ics[model_name]:.0f}, "
            ...
            group_results.append(
                {
                    ...
                    "wins": win_counts.get(model_name, 0),
                    "win_pct": 100 * win_counts.get(model_name, 0) / n_group
                    if n_group > 0
                    else 0,
                }
            )
...
def plot_stratified_wins(merged: pd.DataFrame, metric: str, figures_dir: Path) -> None:
    ...
    colors = sns.color_palette("husl", len(models))
    color_map = dict(zip(models, colors, strict=False))
    ...
    for model in models:
        ...
        for xi, h, b in zip(x, heights, bottoms, strict=False):
    ...
    ax.set_title(
        f"Winning Model by Trauma Group ({metric.upper()})",
        fontsize=14,
        fontweight="bold",
    )
    ax.set_ylim([0, 110])
    ...
    for xi, group in enumerate(groups):
        ...
        ax.text(
            xi, 103, f"n={n}", ha="center", va="bottom", fontsize=10, fontstyle="italic"
        )
...
def run_bayesian_comparison(output_dir: Path) -> None:
    ...
    lines: list[str] = [
        ...
        if hasattr(comparison, "to_markdown")
...
def main():
    ...
    parser.add_argument(
        "--m1", type=str, default=None, help="Path to M1 (Q-Learning) MLE results"
    )
    parser.add_argument(
        "--m2", type=str, default=None, help="Path to M2 (WM-RL) MLE results"
    )
    parser.add_argument(
        "--m3", type=str, default=None, help="Path to M3 (WM-RL+kappa) MLE results"
    )
    parser.add_argument(
        "--m5",
        type=str,
    ...
    parser.add_argument(
        "--output-dir",
        type=str,
        default="output/model_comparison",
        help="Output directory for results",
    )
    parser.add_argument(
        "--use-waic", action="store_true", help="Include WAIC/LOO from Bayesian fits"
    )
    parser.add_argument(
        "--bayesian-dir",
        type=str,
        default="output/bayesian_fits",
    ...
    if len(choice_only_dict) < 2 and m4_fits is None:
        ...
        print(
            "Provide paths via --m1/--m2/--m3 or ensure MLE results exist in output/mle/"
        )
        return
    ...
    if m4_fits is not None:
        n_converged = (
            m4_fits["converged"].sum()
            if "converged" in m4_fits.columns
            else len(m4_fits)
        )
        print(
    ...
    print("\n" + "-" * 80)
    print(
        "INFORMATION CRITERIA COMPARISON (Choice-Only Models: M1, M2, M3, M5, M6a, M6b)"
    )
    print("-" * 80)
    ...
    if len(choice_only_dict) >= 2:
        ...
        # Akaike weights
        aic_values = {
            row["model"]: row["aggregate_aic"] for _, row in aic_comparison.iterrows()
        }
        ...
        for _, row in aic_comparison.iloc[1:].iterrows():
            delta = row["delta_aic"]
            print(
                f"    {row['model']} vs {best_aic}: dAIC = {delta:.2f} ({interpret_delta(delta)})"
            )
    ...
    if len(choice_only_dict) >= 2:
        ...
        results = aic_comparison.merge(
            bic_comparison[["model", "aggregate_bic", "delta_bic"]], on="model"
        )
        ...
        if aic_winner == bic_winner:
            ...
            print(
                "      (BIC penalizes complexity more heavily; disagreement "
    ...
    if len(choice_only_dict) >= 2:
        ...
        wins_combined = aic_wins.merge(
            bic_wins[["model", "wins"]].rename(columns={"wins": "bic_wins"}), on="model"
    ...
    if m4_fits is not None:
        ...
        n_m4 = len(m4_fits)
        n_converged_m4 = (
            m4_fits["converged"].sum() if "converged" in m4_fits.columns else n_m4
        )
        print(f"  M4 participants: {n_converged_m4}/{n_m4} converged")
        if n_converged_m4 > 0:
            m4_conv = (
                m4_fits[m4_fits["converged"]]
                if "converged" in m4_fits.columns
                else m4_fits
            )
            ...
            print(
                f"  M4 mean NLL:     {m4_conv['nll'].mean():.2f} +/- {m4_conv['nll'].sem():.2f}"
            )
            print("  Parameter summary (mean +/- SEM):")
            for param in WMRL_M4_PARAMS:
                if param in m4_conv.columns:
                    print(
                        f"    {param:12s}: {m4_conv[param].mean():.3f} +/- {m4_conv[param].sem():.3f}"
                    )
        # Save M4 summary
