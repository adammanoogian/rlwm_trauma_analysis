...
def warmup_jax_compilation(model: str, verbose: bool = True):
    ...
    for _ in range(n_blocks):
        ...
        rewards_blocks.append(
            jax.random.bernoulli(k3, 0.7, (n_trials,)).astype(jnp.float32)
        )
    ...
    if model == "qlearning":
        ...
        # kappa=0.1, kappa_s=0.1 are decoded values (kappa_total=0.2, kappa_share=0.5)
        wmrl_m6b_multiblock_likelihood(
            stimuli_blocks,
            actions_blocks,
            rewards_blocks,
...
def _make_jax_objective_wmrl_m4(
    ...
    if masks_blocks is not None:
        ...
        masks_stacked = jnp.ones(
            (len(stimuli_blocks), stimuli_stacked.shape[1]), dtype=jnp.float64
        )
...
def _gpu_objective_wmrl_m6b(x, stimuli, actions, rewards, masks, set_sizes):
    """
    Unconstrained objective for WM-RL M6b (dual perseveration) with data as explicit args.

    Args:
        x: unconstrained parameter vector, shape (8,) [kappa_total, kappa_share at index 5, 6]
        stimuli, actions, rewards, masks: shape (n_blocks, max_trials)
        set_sizes: shape (n_blocks, max_trials)
...
def fit_all_gpu(
    ...
    if verbose:
        ...
        print("Method: jaxopt.LBFGS + jax.vmap (fully vectorized)")
        print(f"{'=' * 60}\n")
    ...
    for pid in participants:
        ...
        trial_counts.append(
            int(sum(float(np.asarray(m).sum()) for m in pdata["masks_blocks"]))
        )
    ...
    if model == "qlearning":
        ...
        def _run_one(x0, stimuli, actions, rewards, masks, set_sizes, rts):
            params, state = solver.run(
                x0, stimuli, actions, rewards, masks, set_sizes, rts
            )
            nll = objective(params, stimuli, actions, rewards, masks, set_sizes, rts)
            return params, nll, state.error

        _run_all_starts = jax.vmap(
            _run_one, in_axes=(0, None, None, None, None, None, None)
        )
    ...
    if verbose:
        print(
            "Stage 4: Running GPU-vectorized fitting (JIT compiling + executing)...",
            flush=True,
        )
        t0 = time.time()
    ...
    if verbose:
        print(
            "Stage 5: Selecting best results and transforming parameters...", flush=True
        )
    ...
    for i in range(n_participants):
        ...
        n_near_best = (
            int(np.sum(np.abs(finite_nlls - nll) < 1.0)) if len(finite_nlls) > 0 else 0
        )
    ...
    if compute_diagnostics:
        ...
        for i, result in enumerate(results):
            ...
            if model == "qlearning":
                objective_fn = _make_jax_objective_qlearning(
                    pdata["stimuli_blocks"],
                    pdata["actions_blocks"],
                    pdata["rewards_blocks"],
            ...
            # Hessian diagnostics
            hess_diag = compute_hessian_diagnostics(
                objective_fn, x_unconstrained, model
            )
        ...
        if verbose:
            ...
            print(
                f"  Hessian diagnostics complete: {hess_success}/{n_participants} successful ({time.time() - t0:.1f}s)"
            )
    ...
    if verbose:
        ...
        print(
            f"Participants: {n_participants} | Converged: {n_converged} ({conv_rate:.0f}%)"
        )
        if post_mem > 0:
...
def fit_participant_mle(
    ...
    masks_jax = (
        masks_blocks  # Already JAX arrays from prepare_participant_data, or None
    )
    if show_setup_steps:
        print(f"done ({_time.time() - _t0:.2f}s)", flush=True)
    ...
    for i in range(n_starts):
        ...
        try:
            if verbose and i == 0:
                print(
                    f"            Start 1/{n_starts} (JIT compiling)...",
                    end=" ",
                    flush=True,
                )
                _t0 = _time.time()
            ...
            scipy_success = (
                bool(state.success) if hasattr(state, "success") else nll_finite
            )
            success = (
                nll_finite  # For _JaxoptResult.success: finite NLL means usable result
            )

            result = _JaxoptResult(
                x=np.array(result_params), fun=final_nll, success=success
            )
            results.append(result)
            ...
            opt_info = {
                ...
                "fun_evals": int(state.num_fun_eval)
                if hasattr(state, "num_fun_eval")
                else -1,
            }
            ...
            if verbose and i == 0:
                print(
                    f"done ({_time.time() - _t0:.2f}s, NLL={final_nll:.1f})", flush=True
                )
                if n_starts > 1:
    ...
    if verbose:
        ...
        if iteration_stats:
            nlls = [
                s["final_nll"] for s in iteration_stats if np.isfinite(s["final_nll"])
            ]
            ...
            print(
                f"            Optimizer: {n_scipy_conv}/{n_starts} scipy-converged, {n_hit_limit} hit maxiter",
                flush=True,
            )
            print(
                f"            NLLs: best={best_nll:.1f} | {n_within_1} within 1.0 | {n_within_5} within 5.0 | range={nll_range:.1f}",
                flush=True,
            )
        else:
            print(f"done (total: {_opt_elapsed:.1f}s)", flush=True)
...
def prepare_participant_data(
    ...
    for block_num in sorted(pdata["block"].unique()):
        ...
        if (
            model in ("wmrl", "wmrl_m3", "wmrl_m5", "wmrl_m6a", "wmrl_m6b", "wmrl_m4")
            and "set_size" in block_data.columns
        ):
        ...
        if pad_blocks:
            ...
            if set_sizes is not None:
                stimuli_pad, actions_pad, rewards_pad, set_sizes_pad, mask = (
                    pad_block_to_max(
                        stimuli,
                        actions,
                        rewards,
    ...
    if pad_blocks and masks_blocks:
        (
            ...
            if model
            in ("wmrl", "wmrl_m3", "wmrl_m5", "wmrl_m6a", "wmrl_m6b", "wmrl_m4")
            else None,
        )
...
def fit_all_participants(
    ...
    if verbose:
        print(
            f"  Data preparation complete ({time.time() - prep_start:.1f}s)\n",
            flush=True,
        )
    ...
    if n_jobs == 1:
        ...
        if gpu_devices:
            if verbose:
                print(
                    f"GPU detected ({gpu_devices[0]}). Using vectorized GPU fitting path.\n"
                )
            try:
                return fit_all_gpu(
                    data=data,
        ...
        checkpoint_path = (
            _get_checkpoint_path(output_dir, model) if output_dir else None
        )
        completed_pids = set()
        if checkpoint_path and checkpoint_path.exists():
            ...
            if verbose and completed_pids:
                print(
                    f"Resuming from checkpoint: {len(completed_pids)} participants already completed"
                )
        ...
        for i, args in enumerate(participant_args):
            ...
            if pid in completed_pids:
                if verbose:
                    print(
                        f"[{i + 1:3d}/{n_participants}] Skipping participant {pid} (already in checkpoint)"
                    )
                continue
            ...
            mem_before_mb = log_memory_usage(
                f"PRE-FIT participant {i + 1}", verbose=False
            )
            ...
            if verbose:
                ...
                print(
                    f"\n[{timestamp()}] [{i + 1:3d}/{n_participants}] ({pct_complete:5.1f}%) Fitting {pid} ({n_trials} trials){eta_str}",
                    flush=True,
                )
            ...
            mem_after_mb = log_memory_usage(
                f"POST-FIT participant {i + 1}", verbose=False
            )
            ...
            if verbose:
                ...
                nll_str = (
                    f"NLL={result['nll']:.1f}"
                    if not np.isnan(result["nll"])
                    else "NLL=NaN"
                )
                r2_str = (
                    f"R²={result.get('pseudo_r2', 0):.2f}"
                    if "pseudo_r2" in result
                    and not np.isnan(result.get("pseudo_r2", np.nan))
                    else ""
                )
                near_str = (
                    f"near_best={result.get('n_near_best', '?')}"
                    if "n_near_best" in result
                    else ""
                )
                print(
                    f"      Result: {status_indicator} {nll_str}, {r2_str}, {near_str}, total={fit_time:.1f}s",
                    flush=True,
                )
                ...
                if (i + 1) % 10 == 0:
                    ...
                    current_mem_mb = log_memory_usage(
                        f"checkpoint {i + 1}", verbose=False
                    )
                    mem_str = (
                        f"{current_mem_mb / 1024:.2f}GB"
                        if current_mem_mb > 0
                        else "N/A"
                    )
                    ...
                    print(
                        f"    | PROGRESS: {i + 1}/{n_participants} ({100.0 * (i + 1) / n_participants:.0f}%)"
                    )
                    print(
                        f"    | Elapsed: {elapsed_time / 60:.1f}min | Remaining: ~{remaining / 60:.1f}min"
                    )
                    print(
                        f"    | Converged: {n_converged}/{len(results)} ({conv_rate:.0f}%) | Avg: {steady_avg:.1f}s/participant"
                    )
        ...
        try:
            ...
            )(delayed(_fit_single_participant_worker)(args) for args in parallel_args)
        except Exception as e:
            error_msg = str(e)
            if "SIGKILL" in error_msg or "TerminatedWorkerError" in str(
                type(e).__name__
            ):
                print(f"\n{'=' * 60}")
        ...
        fit_times = []
        for _i, (args, result) in enumerate(
            zip(participant_args, results, strict=False)
        ):
            pid = args[0]
            data_dict = args[1]
            if "masks_blocks" in data_dict and data_dict["masks_blocks"] is not None:
    ...
    if verbose:
        ...
        if fit_times:
            ...
            print(
                f"JIT compilation: {jit_time:.1f}s | Avg steady-state: {steady_avg:.1f}s/participant"
            )
        else:
            # Parallel mode
            avg_per_participant = (
                total_time / n_participants if n_participants > 0 else 0
            )
            print(
                f"Avg time per participant: {avg_per_participant:.1f}s (parallel, {n_jobs} workers)"
            )
...
def compute_group_summary(fits_df: pd.DataFrame, model: str) -> pd.DataFrame:
    """
    ...
    converged_df = fits_df[fits_df["converged"]]
...
def main():
    ...
    parser.add_argument(
        "--data", type=str, required=True, help="Path to trial data CSV"
    )
    parser.add_argument(
        "--output", type=str, default="output/mle/", help="Output directory"
    )
    parser.add_argument(
        "--n-starts",
        type=int,
    ...
    if args.use_gpu:
        print(
            f"GPU acceleration: {'enabled' if gpu_available else 'requested but unavailable'}"
        )
    print(
        f"Hessian diagnostics: {'enabled (adds 5-30s per participant)' if args.compute_diagnostics else 'disabled'}"
    )
    ...
    if timing_info and args.limit:
        ...
        if steady_times:
            ...
            print(
                f"Steady-state per participant: {steady_mean:.1f}s ± {steady_std:.1f}s"
            )
            ...
            print(
                f"Rough estimate for {total_n} participants: {estimated / 60:.1f} min"
            )
    ...
    performance_summary = {
        ...
        "avg_time_per_participant_s": timing_info.get("total_time", 0) / n_participants
        if n_participants > 0
        else 0,
        "jit_compilation_time_s": timing_info.get("jit_time"),
        "steady_state_avg_s": np.mean(timing_info.get("steady_state_times", []))
        if timing_info.get("steady_state_times")
        else None,
        "n_jobs": args.n_jobs,
        "peak_memory_mb": timing_info.get("peak_mem_mb"),
        "initial_memory_mb": timing_info.get("initial_mem_mb"),
        "memory_increase_mb": (
            timing_info.get("peak_mem_mb", 0) - timing_info.get("initial_mem_mb", 0)
        )
        if timing_info.get("peak_mem_mb")
        else None,
        "timestamp": datetime.now().isoformat(),
        "seed": args.seed,
        "data_file": args.data,
        "n_converged": int(fits_df["converged"].sum()),
        "convergence_rate": float(fits_df["converged"].sum() / n_participants)
        if n_participants > 0
        else 0,
    }
    ...
    if "hessian_invertible" in fits_df.columns:
        performance_summary["hessian_success_count"] = int(
            fits_df["hessian_invertible"].sum()
        )
        performance_summary["hessian_success_rate"] = float(
            fits_df["hessian_invertible"].mean()
        )
    ...
    for param in param_names:
        ...
        print(
            f"  {param:12s}: {row['mean']:.3f} +/- {row['se']:.3f}  (SD={row['sd']:.3f})"
        )
    ...
    if "hessian_invertible" in fits_df.columns:
        ...
        if "hessian_condition" in fits_df.columns:
            valid_cond = fits_df[fits_df["hessian_condition"].notna()][
                "hessian_condition"
            ]
            if len(valid_cond) > 0:
                ...
                if high_cond > 0:
                    print(
                        f"  Warning: {high_cond} participants with condition > 1000 (identifiability issues)"
                    )
    ...
    if n_failed == 0 and n_converged == n_total:
        ...
        print(
            f"STATUS: COMPLETE - {n_total} participants fit ({n_converged} converged, {n_total - n_converged} did not converge)"
        )
    else:
        print(
            f"STATUS: PARTIAL - {n_total - n_failed}/{n_total} participants fit, {n_failed} FAILED"
        )
        failed_pids = fits_df[fits_df["nll"].isna()]["participant_id"].tolist()
        print(
            f"  Failed participants: {failed_pids[:10]}{'...' if len(failed_pids) > 10 else ''}"
        )
    print(f"{'=' * 60}\n")
