...
def wmrl_m6a_fully_batched_likelihood(
    ...
    *,
    ...
    stimuli, actions, rewards, set_sizes, masks : jnp.ndarray
        Shape (N, B, T).
    alpha, phi, rho, capacity, kappa_s, epsilon : jnp.ndarray
    ...
    def _block_ll(stim, act, rew, ss, mask, ap, ph, rh, cap, ks, e):
    ...
    def _participant_ll(stim, act, rew, ss, mask, ap, ph, rh, cap, ks, e):
        return _over_blocks(
            stim, act, rew, ss, mask, ap, ph, rh, cap, ks, e,
        ).sum()
...
def wmrl_m6a_block_likelihood_pscan(
    ...
    *,
    ...
    alpha, phi, rho, capacity, kappa_s, epsilon : float
...
def wmrl_m6a_hierarchical_model(
    ...
    *,
    ...
    alpha, phi, rho, capacity, epsilon (6, sampled via
    ...
    # ------------------------------------------------------------------
    # kappa_s with optional L2 shift.
    ...
    if kappa_parameterization == "convex":
        ...
        kappa_s_sigma_pr = numpyro.sample(
            "kappa_s_sigma_pr", dist.HalfNormal(0.2)
        )
        ...
        kappa_s_mu_pr = numpyro.sample(
            "kappa_s_mu_pr", dist.Normal(0.0, 0.5)
        )
        kappa_s_sigma_pr = numpyro.sample(
            "kappa_s_sigma_pr", dist.HalfNormal(0.5)
        )
        kappa_s_z = numpyro.sample(
            "kappa_s_z", dist.Normal(0, 1).expand([n_participants])
        )
        kappa_s_raw = (
            kappa_s_mu_pr
            + kappa_s_sigma_pr * kappa_s_z
        ...
        kappa_s = numpyro.deterministic(
            "kappa_s", jnp.clip(kappa_s_raw, min=-1.0, max=1.0)
        )
