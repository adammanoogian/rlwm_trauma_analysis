...
stimuli_blocks = [
    jnp.array(np.random.randint(0, 6, n_trials), dtype=jnp.int32)
    for _ in range(n_blocks)
]
actions_blocks = [
    jnp.array(np.random.randint(0, 3, n_trials), dtype=jnp.int32)
    for _ in range(n_blocks)
]
rewards_blocks = [
    jnp.array(np.random.binomial(1, 0.7, n_trials), dtype=jnp.float32)
    for _ in range(n_blocks)
]
...
try:
    ...
    log("  This confirms jax.jit(solver.run) doesn't work with jaxopt")
...
n_starts_batch = 20
x0_batch = jnp.array(
    [[np.random.randn(), np.random.randn(), -2.0] for _ in range(n_starts_batch)]
)
...
log()
log("Sequential (jit=True):")
log(f"  First call (compile):       {internal_jit_compile:.2f}s")
log(f"  Per start (sequential):     {internal_jit_time / 10:.2f}s")
log(
    f"  20 starts would take:       {internal_jit_compile + 19 * internal_jit_time / 10:.1f}s"
)
log()
log("VMAP (parallel - RECOMMENDED):")
