...
- [ ] **EXEC-02**: Every expected artifact exists on disk after the cold-start run: `models/bayesian/21_prior_predictive/{model}_prior_sim.nc` (6 files), `models/bayesian/21_recovery/{model}_recovery.csv` (6 files), `models/bayesian/21_baseline/{model}_posterior.nc` (6 files) + `convergence_table.csv` + `convergence_report.md`, `models/bayesian/21_l2/{winner}_posterior.nc` + `{winner}_beta_hdi_table.csv` (winner count from 21.5), `models/bayesian/21_l2/scale_audit_report.md` + `averaged_scale_effects.csv`, `reports/tables/model_comparison/{loo_stacking,rfx_bms,winner_betas}.{csv,md,tex}`, and winner-specific forest plot PNGs in `reports/figures/bayesian/21_bayesian/`
- [ ] **EXEC-03**: SLURM accounting (JobID, wall-clock, CPU-hours, GPU-hours, max memory) logged per step in `models/bayesian/21_execution_log.md`; total GPU-hours documented to enable future-budget planning
