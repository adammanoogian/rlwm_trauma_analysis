# Requirements Archive: v4.0 Hierarchical Bayesian Pipeline & LBA Acceleration

**Archived:** 2026-04-19
**Status:** ✅ SHIPPED

This is the archived requirements specification for v4.0. All 71 requirements code-complete on disk; closure guard `validation/check_v4_closure.py` passes 8/8 on the completion commit. Cluster-execution-pending items (K-03, GPU-03, BMS full cold-start) are documented with canonical cold-start entries.
...
**Shipped:** 71 of 71 v1 requirements code-complete (100%).

**Adjusted during implementation:**
- **K parameterization:** Initial bounds [1, 7] refactored to Collins [2, 6] with `phi_approx` transform per Collins 2012/2014/2018 (K-01 research). `parameterization_version="v4.0-K[2,6]-phiapprox"` stamp added via INFRA-06.
- **L2 subscale design:** Initially planned 6 predictors (LEC total + 3 IES-R residualized subscales + LEC-5 physical/sexual/accident subcategories). Actual: 4 predictors (LEC total + IES-R total + IES-R intrusion residual + IES-R avoidance residual). IES-R hyperarousal residual dropped due to exact linear dependence (the three residuals sum to zero). LEC-5 subcategories unavailable from survey instrument. Documented in 16-01-SUMMARY.md.
- **M6b subscale beta coefficients:** Planned ~48 (8 params × 6 predictors); actual 32 (8 params × 4 predictors) after the L2 design reduction above.
- **L2-08 horseshoe prior:** Deferred per plan 16-07 decision. Baseline `Normal(0, 1)` prior converged cleanly; horseshoe not needed. Moved to v5.0 P2 candidate.
- **DEER-02 implementation:** Research outcome NO-GO. Fixed-point iteration did not deliver sufficient speedup for T≈600 sequences. Phase 20 empirical evidence documented in `docs/PARALLEL_SCAN_LIKELIHOOD.md` §DEER. DEER-02 satisfied by documented no-go decision per 20-VERIFICATION.md.

**Added during milestone** (14 new requirements beyond initial 57-row v1 catalog):
- Phase 19 PSCAN-01..06 (associative scan parallelization) — added 2026-04-14 after Phase 18 manuscript complete.
- Phase 20 DEER-01..04 (research phase) — added 2026-04-14 in same extension.
- Phase 21 BMS-01..10 (principled Bayesian model selection pipeline) — added 2026-04-18 after user flagged MLE-preselected-winner circularity.

**Deferred to cluster execution** (code-complete; 5 items):
- K-02, K-03 (constrained-K MLE refit) — cold-start: `bash cluster/12_submit_all_gpu.sh`
- GPU-01, GPU-02, GPU-03 (M4 GPU LBA path and < 12h wall time) — cold-start: `bash cluster/12_submit_all_gpu.sh`
- BMS-01..BMS-10 full pipeline run (code satisfied; cluster cold-start for empirical verification) — cold-start: `bash cluster/21_submit_pipeline.sh`

**Dropped:** None. Every requirement ID in the v1 catalog (+ in-flight additions PSCAN/DEER/BMS) landed as code-complete.

**Traceability integrity verified by** `validation/check_v4_closure.py` (8/8 PASS on final commit). Traceability row count: 71 (per Coverage Summary **Total**). Drift from pre-Phase-22 audit: closed — all 20 drifted Pending rows updated to Complete during 2026-04-19 quick-fix cleanup (commit `71e063d`).

---

*Requirements defined: 2026-04-11 (v1 catalog)*
*Extended: 2026-04-19 (DEER + BMS families added during Phase 22 closure audit)*
*Archived: 2026-04-19 as part of v4.0 milestone completion*
