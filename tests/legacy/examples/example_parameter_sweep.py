...
print(f"Q-Learning grid size: {len(qlearning_grid['alpha']) * len(qlearning_grid['alpha']) * len(qlearning_grid['beta'])} parameter combinations")
...
for alpha in qlearning_grid['alpha']:
    for alpha in qlearning_grid['alpha']:
        for beta in qlearning_grid['beta']:
            for set_size in set_sizes:

                # Print current condition
                print(f"Q-Learning: α+={alpha:.2f}, α-={alpha:.2f}, β={beta:.1f}, set_size={set_size}")
...
for ss in set_sizes:
    ...
    print(f"  Q-Learning (set_size={ss}): α+={best_row['alpha']:.2f}, α-={best_row['alpha']:.2f}, β={best_row['beta']:.1f} → acc={best_row['accuracy_mean']:.3f}")
