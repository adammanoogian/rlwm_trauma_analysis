...
print(
    f"Q-Learning grid size: {len(qlearning_grid['alpha_pos']) * len(qlearning_grid['alpha_neg']) * len(qlearning_grid['beta'])} parameter combinations"
)
print(
    f"WM-RL grid size: {len(wmrl_grid['capacity']) * len(wmrl_grid['rho'])} parameter combinations"
)
    ...
    for alpha_neg in qlearning_grid["alpha_neg"]:
        for beta in qlearning_grid["beta"]:
            for set_size in set_sizes:
                ...
                print(
                    f"Q-Learning: α+={alpha_pos:.2f}, α-={alpha_neg:.2f}, β={beta:.1f}, set_size={set_size}"
                )
...
for ss in set_sizes:
    ...
    print(
        f"  Q-Learning (set_size={ss}): α+={best_row['alpha_pos']:.2f}, α-={best_row['alpha_neg']:.2f}, β={best_row['beta']:.1f} → acc={best_row['accuracy_mean']:.3f}"
    )
...
for ss in set_sizes:
    ...
    best_row = ss_data.loc[best_idx]
    print(
        f"  WM-RL (set_size={ss}): K={best_row['capacity']}, ρ={best_row['rho']:.1f} → acc={best_row['accuracy_mean']:.3f}"
    )
