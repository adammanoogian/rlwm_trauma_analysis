...
try:
    from ipywidgets import (  # noqa: F401
        Dropdown,
        FloatSlider,
        IntSlider,
        fixed,
...
def explore_qlearning_interactive():
    ...
    def run_and_plot(alpha=0.3, beta=3.0, set_size=3, num_trials=100):
        ...
        results = simulate_agent_on_env(
            agent, env, num_trials=num_trials, log_history=False
        )
        ...
        im = ax.imshow(
            q_table[:set_size, :], aspect="auto", cmap="viridis", vmin=0, vmax=1
        )
...
def explore_wmrl_interactive():
    ...
    def run_and_plot(
        alpha=0.2, beta=3.0, capacity=4, w_wm=0.6, set_size=5, num_trials=100
    ):
        ...
        if wm_contents:
            ...
            ax.set_yticks(y_pos)
            ax.set_yticklabels(
                [f"S{s}→A{a}" for s, a in zip(stims, actions, strict=False)]
            )
            ax.set_xlabel("Memory Strength", fontsize=10)
            ax.set_title(
                f"WM Buffer ({len(wm_contents)}/{capacity})",
                fontsize=11,
                fontweight="bold",
            )
            ax.set_xlim([0, 1])
...
def compare_models_interactive():
    ...
    def run_comparison(
        set_size=4, num_trials=100, alpha=0.3, beta=3.0, capacity=4, w_wm=0.6
    ):
        ...
        results_q = simulate_agent_on_env(
            agent_q, env_q, num_trials=num_trials, log_history=False
        )
        ...
        results_wmrl = simulate_wm_rl_on_env(
            agent_wmrl, env_wmrl, num_trials=num_trials, log_history=False
        )
        ...
        ax.set_title(
            f"Model Comparison (set_size={set_size})", fontsize=14, fontweight="bold"
        )
        ...
        print(
            f"  Difference:  {(results_wmrl['accuracy'] - results_q['accuracy']):.1%}"
        )
